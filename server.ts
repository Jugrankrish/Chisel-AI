...
app.post("/api/run-pipeline", (req, res) => {
  ...
  const pythonArgs = [
    ...
    "--text", prompt,
    "--ply", plyPath,
    "--images", imagesDir || "data/tandt/truck/images",
    "--colmap_dir", colmapDir || "data/tandt/truck/sparse/0",
    "--output", outputPath,
    "--sam", "sam_vit_b_01ec64.pth",
    "--llm", llm || "agent",
    "--box_threshold", String(boxThreshold || "0.30"),
    "--text_threshold", String(textThreshold || "0.25"),
    "--ratio", String(ratio || "0.25"),
  ];
  ...
  proc.on("error", (err: Error) => {
    ...
    job.sseClients.forEach((c) => { try { c.end(); } catch (_) {} });
    job.sseClients = [];
  });
